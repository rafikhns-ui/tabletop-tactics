      ...
      <div className="grid gap-2">
        {units.map(unit => (
          <div 
            ...
            style={{
              background: 'hsl(35,25%,16%)',
              borderColor: 'hsl(35,20%,28%)',
            }}
          >
            ...
            <div className="grid grid-cols-2 gap-1 mb-2 text-xs">
              <div style={{ color: 'hsl(40,20%,60%)' }}>
                <span style={{ color: 'hsl(43,80%,70%)' }}>Move Range:</span> {unit.movementRange} hex
              </div>
              <div style={{ color: 'hsl(40,20%,60%)' }}>
                <span style={{ color: 'hsl(43,80%,70%)' }}>Can Capture:</span> {unit.canCapture ? '✓' : '✗'}
              </div>
            </div>
            ...
            <div className="flex gap-2 text-xs">
              {Object.entries(unit.cost).map(([resource, amount]) => (
                <div key={resource} style={{ background: 'rgba(0,0,0,0.3)', padding: '4px 6px', borderRadius: '3px', color: 'hsl(43,80%,70%)' }}>
                  {resource === 'gold' && '🪙'} {resource === 'wheat' && '🌾'} {resource === 'wood' && '🪵'} {amount}
                </div>
              ))}
            </div>
          </div>
